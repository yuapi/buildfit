...
### 9.3.4 칩셋으로 출시 연도를 메우는 안 — 기각 (2026-09-22)

규칙 12의 병목이 메인보드 연도(79.5% 결측)이고, **칩셋은 99.9% 채워져 있다**
(3,697/3,701). 그럼 칩셋으로 연도를 좁힐 수 있지 않을까.

논리는 이렇다. B650 보드는 B650 출시보다 먼저 나올 수 없다. 그러니 같은 칩셋
보드들의 **관측된 최소 연도**를 하한으로 삼으면, `CPU 연도 ≤ 하한`인 경우
규칙 12를 통과로 판정할 수 있다. ADR-0013이 케이스 폼팩터로 한 것과 비슷하다.

**측정해 보고 기각했다.**

| 칩셋 | 보드 | 연도 표본 | 최소 | 최대 |
|---|---|---|---|---|
| Intel B760 | 204 | 67 | 2022 | 2026 |
| AMD B850 | 147 | 117 | 2024 | 2026 |
| Intel Z790 | 144 | 50 | 2022 | 2024 |
| **AMD B650** | 138 | 58 | **2020** | 2026 |
| AMD B550 | 130 | 47 | 2020 | 2024 |
| AMD B450 | 99 | 19 | 2018 | 2022 |

**B650의 최소가 2020이다.** B650은 그때 존재하지 않았다 — 관측된 최소가
**틀린 값에 오염되어 있다.** 하한으로 쓰면 그 오염이 그대로 판정에 들어가고,
틀린 하한은 **거짓 통과**를 만든다. 이 도구가 낼 수 있는 가장 나쁜 결과다.

141개 칩셋 중 연도 표본이 있는 것은 50개뿐이고, 그중 25개는 관측 범위가
2년 이상 벌어져 있다. 표본이 얇아 오염을 걸러낼 방법도 없다.

### 9.3.4.1 반대 방향으로는 신호가 되는가 — 너무 작다

"칩셋 중앙값보다 2년 이상 이른 연도는 의심한다"로 뒤집어 재 봤다.
표본 5건 이상인 칩셋에서 **4건**이 걸리고, 그중 둘은 참(`ASUS B650M-AYW WIFI`
계열이 2020년으로 적혀 있다), 둘은 거짓이다 (`Intel H410`은 실제로 2020년
출시이고 표본 5건의 중앙값이 2022로 밀린 것이다).

4건 중 2건을 잡는 장치를 넣을 가치는 없다. 중복 레코드 간 불일치(이슈 #11)가
127쌍을 잡는 것과 비교하면 신호가 너무 약하다.

**그래서 규칙 12는 메인보드 연도가 채워질 때까지 판정 불가로 남는다.**
어드민이 채울 수 있게는 해 두었다 (이슈 #15) — 다만 연도 없는 2,944건 중
제조사 주소가 있는 것은 33건(1.1%)뿐이다.

### 9.3.5 규칙 20을 넣은 뒤 (2026-09-22)

규칙 20(쿨러가 CPU 소켓을 지원하는가, 이슈 #17)을 넣고 같은 방식(무작위 300 조합)으로
다시 쟀다. 다른 규칙들은 표본 오차 안에서 §9.3.2와 같다.

| 규칙 | 통과 | 경고 | 판정 불가 |
|---|---|---|---|
| #20 쿨러 소켓 | 186 | 114 | **0 (0.0%)** |

**판정 불가가 없는 규칙이다.** 쿨러 99.9%가 지원 소켓 목록을 갖고 있고 CPU 소켓은
100%다. 경고 38%는 무작위 표본이라 옛 소켓 CPU가 섞인 값이다 — 전수 추정 39.1%와
맞는다. 현행 소켓만 보면 AM5 27.0% · LGA 1700 28.1% · LGA 1851 33.0%다
(`docs/compat-rules.md` §20.1).

이 규칙은 **새 판정 불가를 하나도 더하지 않고** 지금까지 조용히 지나가던 조합을
잡는다. 규칙 15·16과 같은 성격이다.

...
**스펙 수정 제안 (문서 먼저, CLAUDE.md 작업 규칙 2)**

- [ ] §5.1 GPU 대응 전략 → AIB 전량 적재로 수정 (§7.1)
- [ ] §4.1 규칙 7의 필요 데이터에서 "보드 파워" 삭제, 상수 가정 명시 (§5)
- [ ] §4.3 판정 등급에 **"판정 불가"** 추가 (§7.5)
- [ ] §4.2 규칙 10(라디에이터) Phase 2 이후로 이동 검토 (§7.2)
- [ ] §5.1 메인보드 BIOS 우회책에 `flashback` 신호 추가 (§7.4)
- [ ] §5.7.2 GS1 코리안넷 우선순위 하향 (§7.6)

**ADR 후보**

- [ ] 결측 필드 처리 원칙 — "판정 불가" 등급 도입 (§7.5). 사용자 피해와 직결되므로
      ADR로 남길 가치가 있다

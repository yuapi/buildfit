...
- **한국 누진세 기반 전기요금** — 가구 전체 사용량에 PC 사용분을 얹어 누진 구간이
  올라가는지까지 계산한다 (Phase 1, 요금표 확보 대기)
...
| | 상태 |
|---|---|
| 부품 카탈로그 | 22,962건 / 스펙 196,854건 / 표기 변형 55,433건 |
| 호환성 규칙 | 10개 동작 (1~9, 12) |
| 남은 규칙 | 10·11·13·14 — 1차 소스에 필드가 없다 ([이슈 #4](../../issues/4)) |
| 국내 유통 매칭 | ⛔ 네이버 쇼핑 API 키 대기 ([이슈 #1](../../issues/1)) |
| 전기요금 | ⛔ 한전 요금표 확보 불가 ([이슈 #2](../../issues/2)) |
| 스토리지 슬롯 | 보류 — 전력 상수 미확보 ([이슈 #5](../../issues/5)) |
...
| `/admin` | 빈 필드 보강. ⚠️ 접근 제어가 없다 ([이슈 #7](../../issues/7)) |
...
현재 적재 규모는 부품 22,962건 / 스펙 196,854건이며 16초쯤 걸린다.

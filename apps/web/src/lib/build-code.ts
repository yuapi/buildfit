...
export const BUILD_CODE_VERSION = 1;
 ...
 */
const SLOTS = ['cpu', 'motherboard', 'gpu', 'pcCase', 'psu'] as const;
type Slot = (typeof SLOTS)[number];

/** 메모리는 개수가 가변이라 비트 5를 따로 쓴다. */
const RAM_BIT = 1 << SLOTS.length;
...
export function decodeBuildCode(code: string): BuildSelection | null {
  ...
  // 모르는 버전은 조용히 실패한다. v2가 생겨도 v1 디코더는 이 자리에 그대로 둔다.
  if (buf[0] !== BUILD_CODE_VERSION) return null;
  ...
  for (let i = 0; i < SLOTS.length; i += 1) {
    ...
    out[SLOTS[i]!] = bytesToUuid(buf, at);
    at += 16;
  }
